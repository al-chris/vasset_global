...
class UserSettings(db.Model):
    ...
    def __repr__(self):
        return f'<ID: {self.id}, user_id: {self.vasset_user_id} >'
    ...
    @classmethod
    def create_user_settings(cls, vasset_user_id):
        default_settings = cls(vasset_user_id=vasset_user_id)
...
class SecuritySetting(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    two_factor_method = db.Column(db.Enum(TwoFactorMethod), default=TwoFactorMethod.NONE, nullable=False) # either email, phone or google_auth_app.
    ...
    def __repr__(self):
        return f'<ID: {self.id}, 2FA_method: {self.two_factor_method.value}>'
